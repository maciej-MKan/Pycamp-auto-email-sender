...
class DataBase:
    def __init__(self, db_name):
        self.dbase = db_name
        self.connect = sqlite3.connect(self.dbase)
        self.db_connect()
    ...
    def check_if_exists(self):
        if self.dbase == ':memory:' or not path.isfile(self.dbase):
            self.create_db()
        return True
    ...
    def get_content(self, column = 'id', data = None):
        ...
        if data:
            self.cursor.execute(
                f"SELECT id, name, mail, book_name, date FROM book WHERE {column} IS ?", (data,))
        else:
